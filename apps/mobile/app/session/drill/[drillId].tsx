...
export default function DrillDetailScreen() {
  ...
  const latestDrill = useGenerateStore((s) => s.latestDrill) as any;
  const [tab, setTab] = useState<DrillTab>('setup');
  const [showDiagram, setShowDiagram] = useState(false);

  const drills = useMemo(() => session?.drills || session?.json?.drills || [], [session]);
  const index = Number(drillId);
  const drillFromIndex = Number.isFinite(index) ? drills[index] : null;
  const drillFromId =
    drills.find((item: any) => String(item?.id) === String(drillId) || String(item?.refCode) === String(drillId)) ||
    null;
  const drill =
    drillFromId ||
    drillFromIndex ||
    (latestDrill && (String(latestDrill?.id) === String(drillId) || String(latestDrill?.refCode) === String(drillId))
      ? latestDrill
      : null);
  ...
  const setupSteps = asList(drill?.organization?.setupSteps);
  ...
  const storedDrillId = drill?.id || drill?.refCode || (Number.isFinite(index) ? null : drillId);
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>{drill.title || `Drill ${drillId}`}</Text>
        <Text style={styles.meta}>
          {drill.drillType || 'Drill'} · {drill.durationMin || drill.duration || '--'} min
          {drill.refCode ? ` · ${drill.refCode}` : ''}
        </Text>
        ...
        <View style={styles.card}>
          ...
          <StoredDrillDiagram drillId={storedDrillId} fallback={fallbackDiagram} />
        </View>
          ...
          {tab === 'coaching' ? (
            coachingPoints.length ? (
              coachingPoints.map((point, idx) => (
                <Text key={`${idx}-${point}`} style={styles.line}>
                  {idx + 1}. {point}
                </Text>
              ))
            ) : (
        ...
        <SafeAreaView style={styles.modalSafe}>
          ...
          >
            <View style={styles.modalDiagram}>
              <StoredDrillDiagram
                drillId={storedDrillId}
                height={420}
                fallback={
                  <Svg height={420} width={960} viewBox="0 0 100 70">
