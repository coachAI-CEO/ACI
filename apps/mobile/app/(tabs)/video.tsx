      ...
      <View style={styles.selectorRow}>
        {options.map((option) => (
          <Text
            key={`${label}-${option}`}
            onPress={() => {
              if (!locked) onChange(option);
            }}
            style={[styles.selectorChip, value === option ? styles.selectorChipActive : null, locked ? styles.selectorChipLocked : null]}
          >
            {option.replaceAll('_', ' ')}
          </Text>
        ))}
...
export default function VideoTab() {
  ...
  const [coachLevel, setCoachLevel] = useState<CoachLevel>('USSF_C');
  const [formationUsed, setFormationUsed] = useState('4-3-3');
  const [gameModelId, setGameModelId] = useState<GameModelId>(enforcedGameModelId || 'PRESSING');
  ...
  useEffect(() => {
    if (enforcedGameModelId) {
      setGameModelId(enforcedGameModelId);
    }
  }, [enforcedGameModelId]);
  ...
  const run = async () => {
    ...
    try {
      ...
      if (runRequestIdRef.current !== requestId) return;
      setError(describeApiError(err, 'Video analysis failed.'));
      setStatus(null);
    } finally {
      clearTimeout(timeoutId);
      if (runRequestIdRef.current === requestId) setIsRunning(false);
    }
  };
  ...
  const saveLatest = async () => {
    ...
    setIsSaving(true);
    try {
      const ctx = current.latestContext as Record<string, any>;
      await saveVideoAnalysis({
        title: `Video Analysis ${new Date().toLocaleDateString()}`,
        ageGroup: String(ctx.ageGroup || ''),
        playerLevel: String(ctx.playerLevel || ''),
        coachLevel: String(ctx.coachLevel || ''),
        gameModelId: String(ctx.gameModelId || ''),
        phase: String(ctx.phase || ''),
        zone: String(ctx.zone || ''),
        focusTeamColor: String(ctx.focusTeamColor || ''),
        opponentTeamColor: String(ctx.opponentTeamColor || ''),
        sourceFileUri: ctx.sourceFileUri ? String(ctx.sourceFileUri) : undefined,
        fileUriUsed: ctx.fileUriUsed ? String(ctx.fileUriUsed) : undefined,
      ...
      setError(describeApiError(err, 'Could not save analysis.'));
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handoffToGenerate = () => {
    setActiveType('session');
    patchForm({
      ageGroup: ageGroup.replace(/\s.+$/, '') || 'U14',
      playerLevel,
      coachLevel,
      gameModelId,
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>
          {isOnline ? 'Capture or upload a clip, then run analysis.' : 'Video analysis requires an internet connection.'}
        </Text>
        {user?.clubName ? <Text style={styles.subtitle}>Club: {user.clubName}</Text> : null}

        <View style={styles.actionRow}>
          <Button title="Library" onPress={() => void pickFromLibrary()} variant="secondary" />
        ...
        <View style={styles.actionRow}>
          <Button title={dryRun ? 'Dry Run: ON' : 'Dry Run: OFF'} onPress={() => setDryRun((v) => !v)} variant="secondary" />
          <Button title="Save Latest" onPress={() => void saveLatest()} loading={isSaving} variant="secondary" />
        </View>

        <Selector label="Player Level" value={playerLevel} options={['BEGINNER', 'INTERMEDIATE', 'ADVANCED']} onChange={(v) => setPlayerLevel(v as PlayerLevel)} />
        <Selector label="Coach Level" value={coachLevel} options={COACH_LEVELS} onChange={(v) => setCoachLevel(v as CoachLevel)} />
        <Selector
          label="Game Model"
          value={gameModelId}
          options={modelOptions.length ? modelOptions : GAME_MODEL_IDS}
          onChange={(v) => setGameModelId(v as GameModelId)}
          locked={Boolean(enforcedGameModelId)}
        />
        ...
        <Button title="Build corrective session" onPress={handoffToGenerate} variant="secondary" />

        <View style={styles.vaultSection}>
          <Text style={styles.vaultTitle}>Saved Analyses</Text>
          {analysesQuery.error ? <ErrorMessage message={describeApiError(analysesQuery.error)} /> : null}
          ...
          {!analysesQuery.data?.length && !analysesQuery.error ? <Text style={styles.empty}>No saved analyses yet.</Text> : null}
        </View>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  container: { gap: 12, padding: 14, paddingBottom: 28 },
  title: { color: colors.text, fontSize: 22, fontWeight: '700' },
  subtitle: { color: colors.muted },
  actionRow: { flexDirection: 'row', gap: 8 },
  selectorWrap: { gap: 6 },
  selectorLabel: { color: colors.muted, fontSize: 11, textTransform: 'uppercase' },
  ...
  selectorChipActive: { borderColor: colors.primary, color: colors.primary, fontWeight: '700' },
  selectorChipLocked: { opacity: 0.75 },
  status: { color: colors.primary, fontWeight: '600' },
  vaultSection: { gap: 8, marginTop: 6 },
  ...
  savedMeta: { flex: 1, paddingRight: 10 },
  savedRef: { color: colors.primary, fontSize: 12, fontWeight: '700' },
  savedTitle: { color: colors.text, fontSize: 14 },
  savedDelete: { color: colors.danger, fontWeight: '600' },
