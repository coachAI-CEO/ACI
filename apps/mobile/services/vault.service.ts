...
export async function getVaultSession(sessionId: string): Promise<VaultSession> {
  try {
    const response = await api.get<{ ok: boolean; session: VaultSession }>(
      `/vault/sessions/${encodeURIComponent(sessionId)}`
    );
