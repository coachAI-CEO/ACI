  ...
  return [
    ...
    JSON.stringify({
      ...
      organization: {
        ...
        area: { lengthYards: 35, widthYards: 25, notes: "Attacking third of field" },
    ...
    "",
    "⚠️ FINAL CHECKLIST - ALL must be TRUE or structure/clarity will score ≤2:",
    "1. ✓ organization is an OBJECT (not string) with: setupSteps (5-8 verb-starting), area (numeric yards), rotation, restarts, scoring",
    "2. ✓ setupSteps covers ALL: space setup, goal placement, player distribution, starting positions, restart location",
    "3. ✓ description is ≥40 chars and explains what players DO (actions, not just objective)",
    "4. ✓ coachingPoints has ≥3 specific, actionable items (coachable cues, not generic advice)",
    "5. ✓ loadNotes.structure is explicit format: '6 x 2:00 / 1:00 rest (1:0.5)' or similar",
    "6. ✓ ALL age mentions = " + input.ageGroup + " EXACTLY (check description, organization.setupSteps, organization.area.notes, loadNotes.rationale, coachingPoints)",
    "7. ✓ Player counts IDENTICAL: description text = organization.setupSteps text = diagram.players count = numbersOnField.totalPlayersOnField",
    "8. ✓ Using 'diagram' (not 'diagramV1') and 'progressions' array (not 'progression')",
    "",
    "OUTPUT: Raw JSON only (no markdown wrapper, no ```json). Verify ALL 8 checklist items above before outputting."
  ].join("\n");
}
...
export function buildQAReviewerPrompt(drill: any): string {
  ...
  return [
    ...
    "",
    "CLARITY (score 5 ONLY if ALL are true, score 2 if ANY are false):",
    "✓ organization.setupSteps exists with 5-8 items, each starts with verb",
    "✓ organization.area has numeric lengthYards AND widthYards (both numbers, not strings)",
    "✓ organization.rotation is a clear string explaining who rotates when",
    "✓ organization.restarts is a clear string explaining how ball restarts",
    "✓ organization.scoring is a clear string explaining scoring rules",
    "✓ NO age mismatches (all age mentions = drill.ageGroup exactly)",
    "✓ Player counts consistent: description = organization.setupSteps = diagram.players = numbersOnField",
    "✓ NO forbidden keys: has 'diagram' (not 'diagramV1'), has 'progressions' array (not 'progression')",
    "",
    "Score 2 if: organization missing fields, area not numeric, age mismatch, count inconsistency, or forbidden keys present.",
    "",
    "DRILL JSON:",
    prettyDrill
  ].join("\n");
