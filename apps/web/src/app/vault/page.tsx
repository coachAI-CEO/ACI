                  ...
                  {userFeatures?.canExportPDF && (
                    <div className="relative flex items-center rounded-full overflow-hidden border border-emerald-500 bg-emerald-500 text-slate-950 text-sm font-semibold">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 hover:bg-emerald-400 transition-colors"
                      >
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            const a = document.createElement("a");
                            a.href = url;
                            a.download = `session-${selectedSession.title.replace(/[^a-z0-9]/gi, "-")}-compact.pdf`;
                            document.body.appendChild(a); a.click();
                            window.URL.revokeObjectURL(url); document.body.removeChild(a);
                          } catch (e: any) { alert("Error exporting PDF: " + e.message); }
                        }}
                        title="Landscape coach's sheet — 4 drill columns, print-ready"
                        className="flex items-center gap-1.5 px-3 py-2 hover:bg-emerald-400 transition-colors"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
