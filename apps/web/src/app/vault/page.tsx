              ...
              <div className="mt-6 space-y-6">
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-700/50">
                  ...
                  {userFeatures?.canExportPDF && (
                    ...
                    <div className="relative flex items-center rounded-full overflow-hidden border border-emerald-500 bg-emerald-500 text-slate-950 text-sm font-semibold">
                      {/* Primary button — Full PDF */}
                      <button
                        disabled={Boolean(exportingPdf)}
                        onClick={async () => {
                          if (exportingPdf) return;
                          setExportingPdf("full");
                          try {
                            const accessToken = typeof window !== "undefined" ? localStorage.getItem("accessToken") : null;
                            const headers: Record<string, string> = { "Content-Type": "application/json" };
                            if (accessToken) headers["Authorization"] = `Bearer ${accessToken}`;
                            const response = await fetch(`/api/export-session-pdf?rev=${SESSION_PDF_REVISION}`, {
                              ...
                              cache: "no-store",
                              headers,
                              body: JSON.stringify({ session: { ...selectedSession.json, id: selectedSession.id }, format: "full" }),
                            });
                            if (!response.ok) { const err = await response.json(); alert("Error: " + (err.error || "Unknown error")); return; }
                            const blob = await response.blob();
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement("a");
                            a.href = url;
                            a.download = `session-${selectedSession.title.replace(/[^a-z0-9]/gi, "-")}-${SESSION_PDF_REVISION}.pdf`;
                            document.body.appendChild(a); a.click();
                            window.URL.revokeObjectURL(url); document.body.removeChild(a);
                          } catch (e: any) { alert("Error exporting PDF: " + e.message); }
                          finally { setExportingPdf(null); }
                        }}
                        className="flex items-center gap-1.5 px-4 py-2 hover:bg-emerald-400 transition-colors disabled:opacity-60"
                      >
                      ...
                      {/* Compact button — Coach's Sheet */}
                      <button
                        disabled={Boolean(exportingPdf)}
                        onClick={async () => {
                          if (exportingPdf) return;
                          setExportingPdf("compact");
                          try {
                            const accessToken = typeof window !== "undefined" ? localStorage.getItem("accessToken") : null;
                            const headers: Record<string, string> = { "Content-Type": "application/json" };
                            if (accessToken) headers["Authorization"] = `Bearer ${accessToken}`;
                            const response = await fetch(`/api/export-session-pdf?rev=${SESSION_PDF_REVISION}`, {
                              ...
                              cache: "no-store",
                              headers,
                              body: JSON.stringify({ session: { ...selectedSession.json, id: selectedSession.id }, format: "compact" }),
                            });
                            if (!response.ok) { const err = await response.json(); alert("Error: " + (err.error || "Unknown error")); return; }
                            const blob = await response.blob();
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement("a");
                            a.href = url;
                            a.download = `session-${selectedSession.title.replace(/[^a-z0-9]/gi, "-")}-compact-${SESSION_PDF_REVISION}.pdf`;
                            document.body.appendChild(a); a.click();
                            window.URL.revokeObjectURL(url); document.body.removeChild(a);
                          } catch (e: any) { alert("Error exporting PDF: " + e.message); }
                          finally { setExportingPdf(null); }
                        ...
                        className="flex items-center gap-1.5 px-3 py-2 hover:bg-emerald-400 transition-colors disabled:opacity-60"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
