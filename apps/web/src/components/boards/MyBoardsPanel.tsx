...
type BoardFilter = "all" | "private" | "club" | "forked" | "favorites";
...
export default function MyBoardsPanel({ enabled, expanded }: Props) {
  ...
  const [pendingErase, setPendingErase] = useState<TacticalBoardSummary | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return boards.filter((b) => {
      ...
      if (filter === "favorites" && !b.favorited) return false;
  ...
  const counts = useMemo(
    () => ({
      ...
      favorites: boards.filter((b) => b.favorited).length,
    }),
    [boards]
  );
  ...
  const onToggleFavorite = async (e: MouseEvent, board: TacticalBoardSummary) => {
    e.preventDefault();
    e.stopPropagation();
    const next = !board.favorited;
    setBoards((prev) => prev.map((b) => (b.id === board.id ? { ...b, favorited: next } : b)));
    try {
      const data = await patchBoard(board.id, { favorited: next });
      if (!data.ok) {
        setBoards((prev) =>
          prev.map((b) => (b.id === board.id ? { ...b, favorited: board.favorited } : b))
        );
        setError(data.message || data.error || "Could not update favorite");
      }
    } catch (err: unknown) {
      setBoards((prev) =>
        prev.map((b) => (b.id === board.id ? { ...b, favorited: board.favorited } : b))
  ...
  const onErase = (e: MouseEvent, board: TacticalBoardSummary) => {
    e.preventDefault();
    e.stopPropagation();
    setPendingErase(board);
  };

  const confirmErase = async () => {
    const board = pendingErase;
    if (!board) return;
    ...
    try {
      ...
      if (!data.ok) {
        setError(data.error || "Could not erase board");
        return;
      }
      setBoards((prev) => prev.filter((b) => b.id !== board.id));
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Could not erase board");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {filtered.map((b) => (
            <div
              ...
              className="rounded-2xl border border-slate-700/70 bg-slate-900/70 p-3 transition-all hover:border-slate-600/70 cursor-pointer"
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <span className="px-2 py-1 rounded text-[10px] font-semibold bg-emerald-600/20 border border-emerald-500/50 text-emerald-300">
                    View
                  </span>
                  <button
                    type="button"
                    onClick={(e) => void onToggleFavorite(e, b)}
                    ...
                    title={b.favorited ? "Remove from favorites" : "Add to favorites"}
                  >
                    <span className="text-xs font-bold">■</span>
                  </button>
                  <button
                    type="button"
                    onClick={(e) => void onErase(e, b)}
                    disabled={deletingId === b.id}
                    className="px-2 py-1 rounded text-[10px] font-semibold bg-rose-600/20 border border-rose-500/50 text-rose-300 hover:bg-rose-600/30 transition-colors disabled:opacity-50"
                    title="Erase board"
                  >
                    {deletingId === b.id ? "…" : "Erase"}
                  </button>
                </div>
              </div>
              <div className="text-[10px] text-slate-400 space-y-1.5">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="px-1.5 py-0.5 rounded bg-cyan-900/30 text-cyan-300 text-[9px] border border-cyan-700/30">
                    {(b.slideCount || 1) === 1 ? "1 slide" : `${b.slideCount} slides`}
                  </span>
                </div>
              </div>
            </div>
      ...
      <ThemedConfirmModal
        ...
        title={pendingErase ? `Erase “${pendingErase.title}”?` : "Erase this board?"}
