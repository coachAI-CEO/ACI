/**
 * Formation catalogs + placement for match boards.
 * Canonical source for web (`board-formations.ts` re-export) and mobile Setup.
 */

import type { PitchFormatId } from './pitch-formats';
import { unstackDiagramPlayers } from './player-spacing';
import type { WebDiagramPlayer, WebDiagramTeam, WebDiagramV1 } from '../types/tactical-board';
...
export const DEFAULT_FORMATIONS: Record<PitchFormatId, { home: FormationId; away: FormationId }> = {
...
export function playerCountOptions(format: PitchFormatId): Array<number | 'all'> {
  const max = format === '7V7' ? 7 : format === '9V9' ? 9 : 11;
 ...
 * Home (ATT): own goal at high y. Away (DEF): own goal at low y.
 */
