/**
 * Shared arrow-geometry helpers for the tactical board.
 *
 * Pure functions on `DiagramPointRef` / `DiagramPlayer` / `DiagramArrow`
 * (the canonical types from `@aci/shared/types/tactical-board`). No DOM,
 * no React, no I/O. Safe to import from the web editor, the API, and
 * the future mobile editor.
 *
 * Previously lived at `apps/web/src/lib/board-lines.ts`; that file is now
 * a thin re-export so the existing `from "@/lib/board-lines"` imports
...
type DiagramPointRef = WebDiagramPointRef;
