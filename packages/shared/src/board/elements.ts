/**
 * Shared practice-element helpers for the tactical board.
 *
 * This was previously duplicated three times:
 *   - `apps/api/src/services/board-elements.ts`
 *   - `apps/web/src/lib/board-elements.ts`
 *   - (the canonical type was hoisted into `@aci/shared` in Phase A)
 *
 * All consumers (web, API, future mobile editor) should import from here.
 *
 * Pure functions on `BoardElement` (from `@aci/shared/types/tactical-board`).
...
export { BOARD_ELEMENT_KINDS } from "../types/tactical-board";
export type { BoardElement, BoardElementKind };
...
export function facingRotation(from: { x: number; y: number }, to: { x: number; y: number }): number {
  const dx = to.x - from.x;
  const dy = to.y - from.y;
  if (Math.hypot(dx, dy) < 0.4) return 0;
  // 0° = +y (right on the board), 90° = +x (top of screen)
