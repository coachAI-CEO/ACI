...
- **Open on tactical board (`createForkSessionBoard`).** Web forks the session into a board when feature `tacticalBoardV1` is enabled (`page.tsx:3170-3198`). Mobile create sheet supports **From a session** / **From a drill** forks (`CreateBoardSheet` → `FORK_SESSION` / `FORK_DRILL`) when boards are enabled.
